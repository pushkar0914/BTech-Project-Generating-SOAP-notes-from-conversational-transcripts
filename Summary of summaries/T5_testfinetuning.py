...
MODEL_DIR = "./fine_tuned_t5"   # your saved checkpoint from Trainer.train()
tokenizer = T5Tokenizer.from_pretrained(MODEL_DIR)
model     = T5ForConditionalGeneration.from_pretrained(MODEL_DIR).to(device)
model.eval()

# ─── 2) Summarization Function ───────────────────────────────────────────────────
def summarize_sessions(session_texts, 
    ...
    """
    # 2.1) build the single input string
    input_str = "summarize: " + "\n\n".join(session_texts)

    # 2.2) tokenize + move to device
    encoded = tokenizer(
        input_str,
        return_tensors="pt",
        truncation=True,
    ...
    # 2.4) decode and return
    return tokenizer.decode(out_ids[0], skip_special_tokens=True).strip()

# ─── 3) Example: Summarize One Patient by CSV Files ──────────────────────────────
def summarize_patient_from_csv(prefix, csv_folder="NEW_SESSIONS"):
    """
    Reads all CSVs named "<prefix>_TRANSCRIPT*.csv" in csv_folder,
    builds session_texts, then calls summarize_sessions().
    """
    paths = sorted(glob.glob(f"{csv_folder}/{prefix}_TRANSCRIPT*.csv"))
    session_texts = []
    ...
    return summarize_sessions(session_texts)

# ─── 4) Usage ─────────────────────────────────────────────────────────────────────
if __name__ == "__main__":
    # Option A: hand–craft session_texts
    example_sessions = [
    ...
    print("Summary (manual):\n", summarize_sessions(example_sessions))

    # # Option B: load from CSVs on disk
    # patient_id = "300"
    # summary = summarize_patient_from_csv(patient_id, csv_folder="TESTING/sessions")
