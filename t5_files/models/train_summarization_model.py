...
summarization_data = []
# We assume that each training session file in classified_utterances corresponds to an intermediate file
...
def process_text(text, section):
    """Format and tokenize the input text for T5 with section conditioning."""
    input_str = f"section: {section} text: {text}"
    return t5_tokenizer(input_str, return_tensors="pt", padding=True, truncation=True, max_length=512)
print("check7")
...
class SummarizationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        target_encoding = t5_tokenizer(target_summary, return_tensors="pt", padding=True, truncation=True, max_length=128)
        return {
...
def collate_fn(batch):
    ...
    input_ids_padded = pad_sequence(input_ids, batch_first=True, padding_value=t5_tokenizer.pad_token_id)
...
def train_t5_model():
    ...
    """Train the T5 summarization model on training sessions."""
    t5_model.train()
    optimizer = AdamW(t5_model.parameters(), lr=1e-4)
    ...
    for epoch in range(3):
        ...
        for i, batch in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            outputs = t5_model(input_ids=batch['input_ids'].to(device),
    ...
    torch.save(t5_model.state_dict(), "models/t5_summarizer.pth")
    print("T5 Summarization Model Saved!")
...
# Load session split info (training only)
with open("data/session_splits.json", "r") as f:
    splits = json.load(f)
train_files = splits["train"]
...
# Max lengths to match custom BART
MAX_INPUT_LENGTH = 1024
MAX_TARGET_LENGTH = 256

def build_training_pairs(input_path, target_path):
...
class SessionLevelDataset(Dataset):
    def __init__(self, file_list):
        ...
        for file_path in file_list:
            ...
            try:
                src, tgt = build_training_pairs(input_file, target_file)
...
def collate_fn(batch):
    ...
    input_encodings = tokenizer(input_texts, return_tensors="pt", padding=True, truncation=True, max_length=MAX_INPUT_LENGTH)
    target_encodings = tokenizer(target_texts, return_tensors="pt", padding=True, truncation=True, max_length=MAX_TARGET_LENGTH)
...
def train():
    ...
    torch.save(model.state_dict(), "models/t5_summarizer_session_level.pth")
    logging.info("Session-level T5 model saved.")
